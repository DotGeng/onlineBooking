				...
				<div class="banShow">
					<table border="1" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td width="315px" class="tdColor">学生学号</td>
							<td width="308px" class="tdColor">学生姓名</td>
							<td width="308px" class="tdColor">座位</td>
							<td width="180px" class="tdColor">操作</td>
						</tr>
						<% int i = 1; %>
						<c:forEach items="${studentBookingCustomeList}" var="studentBooking">
						<tr>
							<td style="display:none;">${studentBooking.studentbookingid }</td>
							<td><%=i++ %></td>
							<td>${studentBooking.studentno }</td>
							<td>${studentBooking.studentName }</td>
							<td>${studentBooking.seatName }</td>
							<td><%-- <a href="<%=path %>/ModifyProctorPage.action?examID=${proctor.examid }
								"><img class="operation"
									src="<%=path %>/style/img/update.png"></a> --%>
									<a href="javascript:if(confirm('确实要删除吗?'))location='<%=path %>/deleteStudentBookingById.action?studentBookingId=${studentBooking.studentbookingid }'">
