				 ...
				 for(int i = 1 ; i <= count;i ++) {
					...
					for(int j = (i-1)*7+1; j <= 7*i ;j ++) {
						...
						if(list.get(j-1).equals("0")) {
							out.println("<td >");
								out.println("<a  href="+"settingClassroomState.action?state=0"+"&examroomId="+ 
								examroomId +"&batch="+batch+"&week="+j%7+"&period="+i+">设定</a>");
							out.println("</td>");
						}else if(list.get(j-1).equals("2")) {
							out.println("<td bgcolor='e91e6f'>");
								out.println("有课<br>");
								out.println("<a  href="+"settingClassroomState.action?state=2"+"&examroomId="+ 
								examroomId +"&batch="+batch+"&week="+j%7+"&period="+i+">设定</a>");
							out.println("</td>");
						}else {
							out.println("<td >");
								...
								out.println("<br/>");
								//String name = request.getAttribute("classroomName").toString();
								...
								out.println("<br/>"); 
								out.println("<a  href="+"settingClassroomState.action?state=1&teacherName="+list.get(j-1)+"&examroomId="+ 
								examroomId +"&batch="+batch+"&week="+j%7+"&period="+i+">设定</a>");
							out.println("</td>");
						}
					}
